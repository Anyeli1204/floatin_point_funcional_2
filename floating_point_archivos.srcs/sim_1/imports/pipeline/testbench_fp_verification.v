...
module testbench_fp_verification;
  ...
  // Guardar todas las se�ales en el archivo de waveform
  initial begin
    $dumpfile("pipeline_dump_fp.vcd"); 
    $dumpvars(0, testbench_fp_verification);  // Solo este m�dulo y sus sub-m�dulos
  end
  
  // Inicializar la prueba
  initial begin
    $display("==========================================");
    $display("Testbench de Verificaci�n FP");
    $display("==========================================");
    reset = 1; 
    # 22;
    reset = 0;
  ...
  // Valores esperados para verificaci�n
  ...
  always @(posedge clk) begin
    if (!reset) begin
      ...
      // Detectar fsw f3, 8(x0) cuando est� en MEM
      if (FPMemWriteM && DataAdr == expected_fsw_address) begin
        ...
        if (FWriteDataM == expected_fsw_data) begin
          $display("✓ VERIFICACION EXITOSA: Dato correcto (5.0)");
          fsw_verified = 1;
        end else begin
      ...
      // Mostrar informaci�n cada 10 ciclos o cuando hay escritura a memoria
      ...
      // Resumen final despu�s de suficientes ciclos
      if (cycle_count == 50) begin
        ...
        if (fsw_detected) begin
          if (fsw_verified) begin
            $display("✓ fsw f6, 8(x0) EJECUTADO CORRECTAMENTE");
      ...
      // Detener la simulaci�n despu�s de 100 ciclos
